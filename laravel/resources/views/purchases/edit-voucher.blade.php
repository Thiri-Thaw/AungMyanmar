                    ...
                    <div class="card-body px-3 py-1">
                        {{-- <div class="row text-center">
                            ...
                            <div class="form-group row">
                                ...
                                <input type="date" class="form-control form-control-sm" name="remind-date" id="p_remind_date" value="{{$purchase->remind_date}}">
                            </div>
                        <div class="form-group row">
                            <div>
                            ...
                            <select name="" id="p_company" class="form-select form-select-sm company_select_box">
                        ...
                        <div class="form-group row">
                            ...
                            <textarea class="form-control form-control-sm" style="resize: none" id="p_remark">{{$purchase->remark}}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="card purchase-invoice">
                    <div class="card-body">
                        <table class="table table-borderless mb-3" id="purchaseInvoiceTable">
                            <tbody>
                                ...
                                <tr class=" border-top border-top-2">
                                    <th>Total</th>
                                    <td>
                                        <input  id="p_total" type="number" class="form-control form-control-sm text-bold text-success" value="{{$total = $subTotal - ($Discount*$subTotal)/100 +($tax*$subTotal)/100}}" readonly>
                                ...
                                <tr class=" border-top border-top-2">
                                    <th>Left</th>
                                    <td>
                                        <input  id="p_left" type="number" class="form-control form-control-sm text-bold text-success" value="{{$total - $paid}}" readonly>
                        ...
                        <div class="d-flex justify-content-end align-items-center">
                            <a class="btn btn-outline-danger me-2" href="{{url()->previous()}}">Cancle</a>
                            <form action="" id="purchaseInvoiceForm">
                                <button type="submit" class="btn btn-lg btn-success px-4">Update</button>
                            </form>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $(document).on('click', '.remove_select_item', function() {
        ...
        $.post("{{route('removeselectededit.item')}}", {
    ...
    $(document).on('submit','#purchaseInvoiceForm',function (e) {
        e.preventDefault();
        $.post("{{route('purchasevoucher.edit')}}",{
            purchase_id:{{$purchase->id}},
            company:$("select#p_company").val(),
            remark:$("textarea#p_remark").val(),
            date:$("#p_date").val(),
            remind_date:$("#p_remind_date").val(),
            tax : $('#p_tax').val() ,
            ...
            function (data) {
                ...
                setTimeout(() => {
                    if(data.count > 0){
                        ...
                        toastr.success('nothing selected!');
                    }
                }, 500);
